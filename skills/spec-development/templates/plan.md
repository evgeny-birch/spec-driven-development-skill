...
| Status | `not-started` / `in-progress` / `completed` / `blocked` — **derived** from `epic.md` §1 Status, maintained by the orchestrator; on divergence the orchestrator stops and asks |
| Created | YYYY-MM-DD |
